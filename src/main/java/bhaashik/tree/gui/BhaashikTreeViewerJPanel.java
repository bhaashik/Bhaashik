...
import bhaashik.util.FileFilterImpl;
import bhaashik.util.ImageUtil;
 ...
 * @author  anil
 */
public class BhaashikTreeViewerJPanel extends javax.swing.JPanel implements ItemListener, TreeViewerEventListener {

//    private Rule columnView;
    protected Rule rowView;
//    private JToggleButton isMetric;
    protected BhaashikJTable tableJTable;
    protected BhaashikMutableTreeNode chunkRootNode;
    ...
    public BhaashikTreeViewerJPanel(BhaashikMutableTreeNode root, int mode, String lang, boolean rev) {
        super();
...
//        buttonCorner.add(isMetric); 
        ...
        tableJTable = new BhaashikJTable(new BhaashikTableModel(), BhaashikJTable.TREE_MODE, root, true);
...
//        Graphics g = tableJScrollPane.getGraphics();
        ...
        rtl = BhaashikLanguages.isRTL(lang);
...
//        tableJTable.setCellSelectionEnabled(true);
        ...
        BhaashikTableModel bhaashikTableModel = new BhaashikTableModel(0, lcount);
        tableJTable.setModel(bhaashikTableModel);

        positionTree(rootNode, 0, lcount / 2);

        if (reverse) {
            rootNode.setValuesInTable(bhaashikTableModel, mode, true);
        } else {
            rootNode.setValuesInTable(bhaashikTableModel, mode);
        }
        ...
        if (!(mode == BhaashikMutableTreeNode.DEPENDENCY_RELATIONS_MODE || mode == BhaashikMutableTreeNode.PHRASE_STRUCTURE_MODE)) {
            renderer.setToolTipText(GlobalProperties.getIntlString("Click_for_node_data"));
        }

        if (bhaashikTableModel.getColumnCount() > 0) {
            tableJTable.setDefaultRenderer(bhaashikTableModel.getColumnClass(0), renderer);
        }
    ...
    public BhaashikTreeViewerJPanel(BhaashikMutableTreeNode root, int mode, String lang) {
        super();
...
//        buttonCorner.add(isMetric); 
        ...
        //Set up the scroll pane.
//        tableJTable = new BhaashikJTable(new BhaashikTableModel(), columnView.getIncrement());
        tableJTable = new BhaashikJTable(new BhaashikTableModel(), BhaashikJTable.TREE_MODE, true);
        tableJTable.prepareCommands();
        JScrollPane tableScrollPane = new JScrollPane(tableJTable);
        tableScrollPane.setPreferredSize(new Dimension(400, 450));
//        tableScrollPane.setMinimumSize(new Dimension(300, 350));
...
//        tableJTable.setCellSelectionEnabled(true);
    ...
    public void setBhaashikTreeJPanel(BhaashikTreeJPanel bhaashikTreeJPanel) {
        this.bhaashikTreeJPanel = bhaashikTreeJPanel;
    }

    public BhaashikTableModel getFSSchema() {
        if (fsSchema == null && bhaashikTreeJPanel != null) {
            fsSchema = bhaashikTreeJPanel.getFSSchema();
        }
    ...
    public void initPopupMenu() {
        if (bhaashikTreeJPanel != null) {
            popupListener = new TreeViewNodeEditPopupListener(tableJTable, treeViewerJPopupMenu, bhaashikTreeJPanel.getNodeLabelEditors(), getFSSchema());
...
// TODO add your handling code here:
    ...
    private void saveJButtonActionPerformed(java.awt.event.ActionEvent evt) {
        if (mode == BhaashikMutableTreeNode.DEPENDENCY_RELATIONS_MODE) {
...
//            ssfChunkRoot.copyDataMM2Chunk((SSFPhrase) ssfMMRoot.getChild(0), cfgToMMTreeMapping);
            ...
            if (bhaashikTreeJPanel != null) {
                ((DefaultTreeModel) bhaashikTreeJPanel.getJTree().getModel()).reload();
//                bhaashikTreeJPanel.getJTree().expandRow(1);
                bhaashikTreeJPanel.collapseAll(null);
...
//                BufferedImage image = new BufferedImage(tableJTable.getWidth(), tableJTable.getHeight(), BufferedImage.TYPE_INT_RGB);
                ...
                Logger.getLogger(BhaashikTreeViewerJPanel.class.getName()).log(Level.SEVERE, null, ex);
            } catch (IOException ex) {
                Logger.getLogger(BhaashikTreeViewerJPanel.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
...
// TODO add your handling code here:
    ...
    private void expandJButtonActionPerformed(java.awt.event.ActionEvent evt) {
        if (collapsed == false || mode != BhaashikMutableTreeNode.DEPENDENCY_RELATIONS_MODE || cfgToDepTreeMapping == null) {
            return;
        }
    ...
    private void collapseJButtonActionPerformed(java.awt.event.ActionEvent evt) {
        if (collapsed || mode != BhaashikMutableTreeNode.DEPENDENCY_RELATIONS_MODE || cfgToDepTreeMapping == null) {
            return;
        }
    ...
    public void setColumnClasses() {
        if (mode == BhaashikMutableTreeNode.DICT_FST_MODE) {
            int ccount = tableJTable.getModel().getColumnCount();

            for (int i = 0; i < ccount; i++) {
                if (owner != null) {
                    BhaashikActionTableCellEditor editor = new BhaashikActionTableCellEditor(owner, language, GlobalProperties.getIntlString("Affix_Editor"), BhaashikActionTableCellEditor.DICTIONARY_FST_MODE);
                    tableJTable.getColumnModel().getColumn(i).setCellEditor(editor);
                } else if (dialog != null) {
                    BhaashikActionTableCellEditor editor = new BhaashikActionTableCellEditor(dialog, language, GlobalProperties.getIntlString("Affix_Editor"), BhaashikActionTableCellEditor.DICTIONARY_FST_MODE);
                    tableJTable.getColumnModel().getColumn(i).setCellEditor(editor);
                }
            }
        }
    ...
    private void positionTree(BhaashikMutableTreeNode node, int level, int pos) {
        ...
        if (mode == SSFPhrase.DEPENDENCY_RELATIONS_MODE) {
            ...
            if (bhaashikTableModel.getRowCount() == level - 1) {
                bhaashikTableModel.addRow();
            }
        } else {
            if (bhaashikTableModel.getRowCount() == level) {
                bhaashikTableModel.addRow();
            }
        }
        ...
        if (node.isLeaf() == false) {
            ...
            for (int i = 0; i < chcount; i++) {
                BhaashikMutableTreeNode chnode = (BhaashikMutableTreeNode) node.getChildAt(i);
        ...
        {
            int colCount = bhaashikTableModel.getColumnCount();
            node.setColumnIndex(colCount - pos - 1);
        }
    }

    public BhaashikMutableTreeNode getRoot() {
        return rootNode;
    }
...
//
//    public tableJTable getBhaashikJTable()
//    {
//        return tableJTable;
//    }
