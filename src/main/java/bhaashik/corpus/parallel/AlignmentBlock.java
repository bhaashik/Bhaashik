...
//    }
    ...
    public boolean isValidAlignment(AlignmentUnit srcAUnit, AlignmentUnit tgtAUnit)
    {
        ...
        while(itr.hasNext())
        {
            ...
            BhaashikEdge e = edgeMap.get(ekey);
    ...
    public void addEdge(AlignmentUnit<T> srcAUnit, AlignmentUnit<T> tgtAUnit)
    {
        ...
        if(mode == SENTENCE_ALIGNMENT_MODE || mode == PARAGRAPH_ALIGNMENT_MODE) {
            e = new BhaashikEdge(srcAUnit, srcAUnit.getIndex(), srcAUnit.getParallelIndex(), tgtAUnit, tgtAUnit.getIndex(), tgtAUnit.getParallelIndex());
        }
        else if(mode == PHRASE_ALIGNMENT_MODE) {
            e = new BhaashikEdge(srcAUnit, srcAUnit.getParallelIndex(), srcAUnit.getIndex(), tgtAUnit, tgtAUnit.getParallelIndex(), tgtAUnit.getIndex());
        }
    ...
    public BhaashikTableModel getAlignmentTable()
    {
        BhaashikTableModel alignmentTable = new BhaashikTableModel(1, 1);
        ...
        if(scount > 0 && tcount > 0)
        {
            ...
            {
                alignmentTable = new BhaashikTableModel(new String[]{"Source Language", "Quality Estimate", "Target Language"}, 0);
            ...
            {
                alignmentTable = new BhaashikTableModel(0, 0);
