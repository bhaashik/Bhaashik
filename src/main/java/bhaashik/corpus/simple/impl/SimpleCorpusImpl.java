package bhaashik.corpus.simple.impl;
...
public class SimpleCorpusImpl extends Corpus implements SimpleCorpus
{
	...
	protected BhaashikTableModel wordTypeTable; //each row of the table represents wordtype of the corpus, the column
	...
	public BhaashikTableModel getWordTypeTable()
	{
	...
	public int read(File file, String charset)throws FileNotFoundException, IOException
	{
		...
		BhaashikTableModel schema = new BhaashikTableModel(schemaFile, dataKVP.getPropertyValue("SchemaCharSet"));
		
		wordTypeTable = new BhaashikTableModel(dataKVP.getPropertyValue("CorpusCharSet"), schema);
		...
		while( (line = lnReader.readLine()) != null )
		{
			...
			for(int i = 0; i < wcount; i++)
			{
				...
				{                    
                    ...
                    if(freqColIndex != -1)
                        wordTypeTable.setValueAt(Long.valueOf(1), rowIndex, freqColIndex);

                    words.put(wrd, Integer.valueOf(rowIndex));
				...
				else
				{
                    ...
                    if(freqColIndex != -1)
                    {
                        ...
                        wordTypeTable.setValueAt(Long.valueOf(freq + 1), rowIndex, freqColIndex);
                    }
