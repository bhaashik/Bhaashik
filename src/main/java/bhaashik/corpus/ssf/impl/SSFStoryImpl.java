...
public class SSFStoryImpl extends SSFTextImpl
        implements Serializable, SSFStory, BhaashikDOMElement
{
    public SSFStoryImpl()
    {
...
//		if(line.matches(sentenceStart))
		{
		    sentencesStarted++;
		    sentenceHash.put(Integer.valueOf(sentencesStarted), Boolean.valueOf(false));
...
//		else if(line.matches(sentenceEnd))
		{
		    ...
		    if(sentenceHash.get(Integer.valueOf(sentencesEnded)) != null) {
                        sentenceHash.put(Integer.valueOf(sentencesEnded), Boolean.valueOf(true));
                    }
...
//            return;
//        }
		else if(corpusType.equals(CorpusType.STANDFORD_PARSER))
		{
			readStafordParserOutput(filePath, cs, corpusType, errorLog);
			return;
		}
		else if(corpusType.equals(CorpusType.CONLLU_FORMAT))
		{
			readCoNLLUFormat(filePath, cs, corpusType, errorLog);
			return;
		}
...
//	    if(lineArray[i].matches(sentenceStart))
	    {
		sentencesStarted++;
		sentenceHash.put(Integer.valueOf(sentencesStarted), false);
	    }
...
//	    else if(lineArray[i].matches(sentenceEnd))
		...
		if(sentenceHash.get(Integer.valueOf(sentencesEnded)) != null) {
                    sentenceHash.put(Integer.valueOf(sentencesEnded), true);
                }
	    }
...
//            else if(lineArray[i].matches(storyEnd) && lines.equals("") == false)
    ...
    public static void replaceWordsNotInListWithTags(SSFStory story, KeyValueProperties pruneWordList)
    {
        ...
        for (int i = 0; i < scount; i++)
        {
            ...
            List<BhaashikMutableTreeNode> words = sentence.getRoot().getAllLeaves();
...
//                            ((Alignable) lexItem).getAlignmentUnit().setIndex(i);
                            ...
                            alignIndices.add(Integer.valueOf(Integer.parseInt(str) - 1));
...
//            story.readFile("/extra/work/questimate/tmp/source.eng", "UTF-8"); //throws java.io.FileNotFoundException;
            story.readFile("/home/anil/bhaash-debug-data/poslcat-single-fs.txt", "UTF-8"); //throws java.io.FileNotFoundException;
