...
public class SSFTextImpl extends Text
        implements Serializable, SSFText, BhaashikDOMElement,
        TypeCraftDOMElement, GATEDOMElement
{
    ...
    public int countCharacters()
    {
        ...
        for (int i = 0; i < scount; i++)
        {
            List<BhaashikMutableTreeNode> leaves = getSentence(i).getRoot().getAllLeaves();
    ...
    public double getAvgTokenLength()
    {
        ...
        for (int i = 0; i < count; i++) {
            ...
            List<BhaashikMutableTreeNode> leaves = sen.getRoot().getAllLeaves();
...
//	clearFeatureStructures();
    ...
    public static void transferTags(SSFText tgtText, SSFText srcText)
    {
        ...
        for (int i = 0; i < scount; i++)
        {
            ...
            List<BhaashikMutableTreeNode> srcWords = srcRoot.getAllLeaves();
            List<BhaashikMutableTreeNode> tgtWords = tgtRoot.getAllLeaves();
...
//        rootNode = XMLUtils.parseDomXML(f);
    ...
    @Override
    public int readStafordParserOutput(String filePath, String charset, CorpusType corpusType, List<String> errorLog /*Strings*/) throws FileNotFoundException, IOException, UnsupportedEncodingException, SAXException, Exception
    {
        BufferedReader lnReader = new BufferedReader(new InputStreamReader(new FileInputStream(filePath), charset));
    ...
    @Override
    public int readCoNLLUFormat(String filePath, String charset, CorpusType corpusType, List<String> errorLog /*Strings*/) throws FileNotFoundException, IOException, UnsupportedEncodingException, SAXException, Exception
    {
        BufferedReader lnReader = new BufferedReader(new InputStreamReader(new FileInputStream(filePath), charset));
...
//            ps.println("<!DOCTYPE frameset PUBLIC \"-//Bhaashik//Frameset//EN\" SYSTEM \"" + "./data/propbank/resource/frameset/frameset.dtd\">");
        ...
        if(count >= 1)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                for (int j = 0; j < scount; j++)
                {
                    ...
                    DOMElement sdomElement = ((BhaashikDOMElement) sentence).getDOMElement();
    ...
    public void readXML(Element domElement) {
        ...
        while(node != null)
        {
            if(node instanceof Element)
            {
                ...
                {
//                        ((BhaashikDOMElement) featureStructure).readXML(element);
                }
...
//        ps.println(getXML());
    ...
    public class SSFParagraph implements Alignable, BhaashikDOMElement
    {
        ...
        public DOMElement getDOMElement() {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                DOMElement idomElement = ((BhaashikDOMElement) sentence).getDOMElement();
