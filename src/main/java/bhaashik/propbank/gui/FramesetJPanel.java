 ...
 * @author anil
 */
public class FramesetJPanel extends javax.swing.JPanel implements WindowListener, FileDisplayer, BhaashikClient, JPanelDialog {
    ...
    private static void loadState(FramesetJPanel editorInstance) {
        stateKVProps = BhaashikClientsStateData.getSateData().getPropertiesValue(ClientType.FRAMESET_EDITOR.toString());
...
//            else
        ...
        if(bhaashikBackup != null)
            FileMonitor.getInstance().removeFileChangeListener(bhaashikBackup, file);
    ...
    public static String switchLanguageStatic(Component parent) {
        ...
        if(switching)
            initLang = BhaashikLanguages.getLanguageName( ((FramesetJPanel) parent).langEnc );
    ...
    protected static String switchEncodingStatic(Component parent, String leCode, EventObject e) {
        ...
        if(switching)
            initEnc = BhaashikLanguages.getEncodingName( ((FramesetJPanel) parent).langEnc );

        String currentLanguage = BhaashikLanguages.getLanguageName( ((FramesetJPanel) parent).langEnc );
...
//		charset = JOptionPane.showInputDialog(parentComponent, "Please enter the charset:", "UTF-8");
                ...
                FileMonitor.getInstance().addFileChangeListener(bhaashikBackup, file, backupPeriod);
