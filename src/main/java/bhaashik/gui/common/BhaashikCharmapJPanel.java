 ...
 * @author anil
 */
public class BhaashikCharmapJPanel extends javax.swing.JPanel
        implements WindowListener, JPanelDialog, BhaashikClient {
    ...
    /** Creates new form BhaashikCharmapJPanel */
    public BhaashikCharmapJPanel() {
        initComponents();
    ...
    protected void init()
    {
        ...
        BhaashikLanguages.fillAllLanguages(langauges);
        ...
        BhaashikLanguages.fillFonts(fonts, langEnc, true);
        ...
        BhaashikLanguages.fillAllScripts(scripts);

        unicodeBlocks = new DefaultComboBoxModel();
        BhaashikLanguages.fillAllUnicodeBlocks(unicodeBlocks);
    ...
    public void setFont(Font fnt)
    {
        ...
        {
            BhaashikCharDisplayPanel bhaashikCharDisplayPanel = (BhaashikCharDisplayPanel) charTableJPanel.getComponent(i);

            bhaashikCharDisplayPanel.setFont(fnt);
        }
    ...
    protected void loadScript()
    {
        ...
        Font font = BhaashikLanguages.getFontFor(BhaashikLanguages.getUnicodeBlockForScript(scriptName));

        if(font != null)
            selFont = font.deriveFont(Font.PLAIN, 24);
        else
            font = BhaashikLanguages.getDefaultLangEncFont("hin::utf8");

        if(selFont == null)
            selFont = BhaashikLanguages.getDefaultLangEncFont("hin::utf8");

        setFontHelper(font);

        Vector<Character> chars = BhaashikLanguages.getSupportedCharacters(scriptName);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            BhaashikCharDisplayPanel bhaashikCharDisplayPanel = new BhaashikCharDisplayPanel();

            bhaashikCharDisplayPanel.setFont(selFont);

            bhaashikCharDisplayPanel.setCharacter(ch.charValue());
            bhaashikCharDisplayPanel.setTextComponent(textJTextField);
            bhaashikCharDisplayPanel.setCharSummaryComponent(statusJLabel);
            bhaashikCharDisplayPanel.setCharDetailsComponent(charDetailsJTextPane);

            charTableJPanel.add(bhaashikCharDisplayPanel);
        }
    ...
    protected void loadUnicodeBlock()
    {
        ...
        Font font = BhaashikLanguages.getFontFor(selBlock);
        ...
        Vector<Character> chars = BhaashikLanguages.getSupportedCharacters(selBlock);
    ...
    protected void loadFont()
    {
        ...
        Vector<Character> chars = BhaashikLanguages.getSupportedCharacters(selFont);
...
//        KeyValueProperties stateKVProps = BhaashikClientsStateData.getSateData().getPropertiesValue(ClientType.SENTENCE_ALIGNMENT_INTERFACE.toString());
//
//        if(srcLangauge != null)
//        {
...
//        }
    ...
    {//GEN-HEADEREND:event_copyJButtonActionPerformed
        ...
        BhaashikStringDataTransfer stringTransfer = new BhaashikStringDataTransfer();
    ...
    private static void saveState(BhaashikCharmapJPanel editorInstance) {
        stateKVProps = BhaashikClientsStateData.getSateData().getPropertiesValue(ClientType.SANCHAY_CHARMAP.toString());
    ...
    private static void loadState(BhaashikCharmapJPanel editorInstance) {
        stateKVProps = BhaashikClientsStateData.getSateData().getPropertiesValue(ClientType.SANCHAY_CHARMAP.toString());

        String langEnc = stateKVProps.getPropertyValue("langEnc");

        if(langEnc == null) {
            langEnc = GlobalProperties.getIntlString("eng::utf8");
    ...
    private static void createAndShowGUI() {
    	...
    	BhaashikCharmapJPanel newContentPane = new BhaashikCharmapJPanel();
