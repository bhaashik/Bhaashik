...
import bhaashik.gui.shell.commands.MathEvaluator;
import bhaashik.text.diff.Report;
import bhaashik.text.diff.TextDiff;
import bhaashik.xml.diff.XMLDiff;
import org.python.core.PyObject;
import org.python.core.PySystemState;
import org.python.util.PythonInterpreter;
import bhaashik.java.JavaDiff;
 ...
 * @author anil
 */
public class BhaashikShellJPanel extends javax.swing.JPanel
        implements JPanelDialog, BhaashikClient, BhaashikShellEventListener {
...
//    protected BhaashikConsoleReader consoleReader;
...
//    protected JDMP
    ...
    /** Creates new form BhaashikShellJPanel */
    public BhaashikShellJPanel() {
        initComponents();
...
//            consoleReader = new BhaashikConsoleReader();
//            consoleReader = new BhaashikConsoleReader(piIn,
//                    new PrintWriter(
//                            new OutputStreamWriter(psOut)));

        } catch (IOException ex)
        {
            Logger.getLogger(BhaashikShellJPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//        new BhaashikConsoleReaderThread(consoleReader, stdOutJTextShell, this).start();
        new BhaashikShellReaderThread(piOut, stdOutJTextShell, this).start();
        new BhaashikShellReaderThread(piErr, stdErrJTextShell, this).start();
...
//            jMathLibInterpreter = new jmathlib.core.jMathLibInterpreter.Interpreter(false);
        ...
        stdOutJTextShell.addBhaashikShellEventListener(this);
    ...
    //                    output += curFile.getAbsolutePath() + "\n";
            ...
            {
                ((BhaashikMain) owner).createNewApplication(ClientType.SANCHAY_EDITOR);
            }
    ...
    //                process = null;
            ...
            {
                ((BhaashikMain) owner).createNewApplication(ClientType.SANCHAY_SHELL);
            }
            else if(cmd.startsWith("new "))
            {
                ...
                else if(parts.length == 2)
                {
                    ...
                    if(modeStr.equals("math"))
                    {
                        ...
                        BhaashikClient client = ((BhaashikMain) owner).createNewApplication(ClientType.SANCHAY_SHELL);
                        ((BhaashikShellJPanel) client).setMode(m);
                    }
...
//                        ((BhaashikShellJPanel) client).setMode(m);
            ...
            {
                ((BhaashikMain) owner).stopApplication();
            }
        }
...
//            addr = InetAddress.getByAddress(ipAddr);
    ...
    @Override
    public void handledShellEvent(BhaashikShellEvent evt)
    {
        int id = evt.getEventID();

        if(id == BhaashikShellEvent.SHELL_COMMAND_EVENT)
        {
    ...
    private static void createAndShowGUI() {
    	...
    	BhaashikShellJPanel newContentPane = new BhaashikShellJPanel();
