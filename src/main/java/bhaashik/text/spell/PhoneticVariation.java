...
import bhaashik.mlearning.common.impl.TranslationListFreqProb;
import bhaashik.speech.common.StringNode;
import bhaashik.speech.common.TrellisNode;
...
//            System.out.println("\t" + srcContextString);
    ...
    public void addContexts(String srcContextString, String tgtContextString, char srcChar, char tgtChar, String srcWord, String tgtWord)
    {
        ...
        if(countInt == null)
            fs_cl_cr.put(contextString, Integer.valueOf(1));
        else
            fs_cl_cr.put(contextString, Integer.valueOf(countInt.intValue() + 1));
        ...
        if(freqProb == null)
            ft_Given_fs_cl_cr.put(contextString, new TranslationListFreqProb(Character.valueOf(srcChar) + " in " + srcWord, new Character(tgtChar) + " in " + tgtWord, 1));
        else
        {
            freqProb.setFrequency(freqProb.getFrequency() + 1);
            freqProb.addObjects(Character.valueOf(srcChar) + " in " + srcWord, Character.valueOf(tgtChar) + " in " + tgtWord);
        }
    }
