 ...
 */
/**
 * The BhaashikMain class provides a GUI for all the Bhaashik applications.
 * 
 */
package bhaashik;
...
import bhaashik.common.types.ServerType;
...
import bhaashik.gui.console.ConsoleJPanel;
import bhaashik.help.HelpApp;
import bhaashik.servers.UserManagerRI;
import bhaashik.gui.clients.*;
...
import bhaashik.common.types.BhaashikType;
...
//@EnableScheduling
public class BhaashikMain extends javax.swing.JFrame implements WindowListener, BhaashikMainEventListener {
    ...
    private BhaashikMainAction actions[];

//    private BhaashikServerRI bhaashikServerRI;
    private UserManagerRI userManagerRI;

//    private LoginJPanel loginJPanel;
    private static BhaashikRemoteWorkJPanel remoteClientJPanel;
    ...
    public static BhaashikMain bhaashikMain;
    public static final int DEFAULT_MODE = 1000;

    /** Creates new form BhaashikMain */
    public BhaashikMain() {
        ...
        setTitle(GlobalProperties.getIntlString("Bhaashik"));
        ImageIcon img = new ImageIcon(GlobalProperties.getHomeDirectory() + "/" + GlobalProperties.getIntlString("images/bhaashik-logo.jpg"), GlobalProperties.getIntlString("Bhaashik"));

        setIconImage(img.getImage());
        initComponents();

        updateJMenuItem.setText("Update"); // NOI18N
        updateJMenuItem.setToolTipText("Update your version of Bhaashik"); // NOI18N
        ...
        currentClientsJList.setCellRenderer(new BhaashikClientListRenderer());
...
//        popoutJPanel.setVisible(false);
        ...
        try {
            ...
            Logger.getLogger(BhaashikMain.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        remoteClientJPanel = new BhaashikRemoteWorkJPanel();

//        sshClientJPanel = (BhaashikSSHClientJPanel) createNewApplication(ClientType.SANCHAY_SSH_CLIENT);
//        createNewApplication(ClientType.SANCHAY_SHELL);
    ...
    private void initComponents() {
        ...
        setTitle(bundle.getString("Bhaashik")); // NOI18N
        setName(bundle.getString("BhaashikJFrame")); // NOI18N
        ...
        aboutJMenuItem.setToolTipText(bundle.getString("About_Bhaashik")); // NOI18N
        ...
        helpJMenuItem.setToolTipText(bundle.getString("Help_about_using_Bhaashik")); // NOI18N
        ...
        updateJMenuItem.setToolTipText(bundle.getString("Help_about_using_Bhaashik")); // NOI18N
        updateJMenuItem.setEnabled(false);
...
//
//    BhaashikJDialog dlg = new BhaashikJDialog((Frame) getOwner(), title, false, pnl);
...
private void helpJMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_helpJMenuItemActionPerformed
    // TODO add your handling code here:
    helpApp = new HelpApp(GlobalProperties.getIntlString("Bhaashik_HelpSet"), GlobalProperties.getIntlString("Bhaashik_HelpSet.hs"));
...
//
//    BhaashikJDialog updateJDialog = null;
//
//    updateJDialog = new BhaashikJDialog(this, "Update Bhaashik", true, (JPanelDialog) updateJPanel);
//
//    updateJDialog.pack();
//
...
private void proxyJMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_proxyJMenuItemActionPerformed
    ...
    {
        JOptionPane.showMessageDialog(this, "Invalid address or port", GlobalProperties.getIntlString("Error"), JOptionPane.ERROR_MESSAGE);
//        ex.printStackTrace();
    }
    
}//GEN-LAST:event_proxyJMenuItemActionPerformed

    public static BhaashikMain getBhaashikMain()
    {
        return bhaashikMain;
    }

    public BhaashikClient getBhaashikClient(ClientType clientType, String file)
    {
        int ccount = inputList.getSize();

        for (int i = 0; i < ccount; i++)
        {
            BhaashikClient client = (BhaashikClient) inputList.elementAt(i);
    ...
    {
        allCommands = new boolean[BhaashikMainAction._TOTAL_ACTIONS_];
        actions = new BhaashikMainAction[BhaashikMainAction._TOTAL_ACTIONS_];

        for(int i = 0; i < allCommands.length; i++)
        {
            allCommands[i] = true;
	    actions[i] = BhaashikMainAction.createAction(this, i);
        }
...
//        URL url = myClass.getResource("BhaashikMain.class");
//
...
//        SpringApplication.run(BhaashikMain.class);

        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
//                SpringApplication.run(BhaashikMain.class);
                new BhaashikMain().setVisible(true);
            }
        });
    }
    ...
    protected void initToolbar()
    {
        Enumeration enm = ClientType.elements();
         while (enm.hasMoreElements()) {
            ...
            BhaashikMainAction act = new BhaashikMainAction(this, clientName, ctype.getCode());
...
//        }
    ...
    public void openTab(BhaashikMainEvent evt)
    {
        if(evt.getClientType() != null)
            createNewApplication(evt.getClientType());
    }

    public void displayFile(BhaashikMainEvent evt)
    {
        BhaashikClient client = null;

        if(evt.getClientType().equals(ClientType.SYNTACTIC_ANNOTATION) && evt.getDisplayObject() != null)
            client = getBhaashikClient(evt.getClientType(), ((SSFStory) evt.getDisplayObject()).getSSFFile());
        else
            client = getBhaashikClient(evt.getClientType(), evt.getFilePath());
        ...
        if(client == null)
        {
            ...
            {
                BhaashikClient bhaashikClient = createNewApplication(evt.getClientType());

                if(bhaashikClient instanceof FileDisplayer)
                {
                    ((FileDisplayer) bhaashikClient).displayFile(evt);
                    activeJTabbedPane.setSelectedComponent((Component) bhaashikClient);
                }
            }
        }
    ...
    public BhaashikClient createNewApplication(ClientType ctype)
    {
...
//            activeJTabbedPane.setSelectedIndex(0);
    ...
    //    setCursor(new Cursor((Cursor.WAIT_CURSOR)));
        ...
        JPanel clientJPanel = ClientType.createBhaashikClient(ctype);
        ...
        ((BhaashikClient) clientJPanel).setOwner(this);
        ((BhaashikClient) clientJPanel).setParentComponent(mainJDesktopPane);
...
//        componentJPopupMenu = ((BhaashikClient) clientJPanel).getJPopupMenu();
//
...
//        }
    ...
    //        {
            ...
            else if(BhaashikClient.class.isAssignableFrom(c.getClass()))
            {
                String title = ((BhaashikClient) c).getTitle();
                title = GlobalProperties.getIntlString("Bhaashik:_") + ClientType.findFromClassName(c.getClass().getName()) + GlobalProperties.getIntlString(":_") + title;
                setTitle(title);
            }
        }                
    }
    ...
    private void populateNewMenu()
    {
        Enumeration enm = ClientType.elements();
         while (enm.hasMoreElements()) {
            ...
            BhaashikMainAction act = new BhaashikMainAction(this, menuText);
...
////        JDialog loginDialog = new JDialog(this, GlobalProperties.getIntlString("Login_to_Bhaashik"), true);
//
//        sshClientJPanel = new BhaashikSSHClientJPanel();
//
...
////            actions[BhaashikMainAction.LOGIN].setEnabled(false);
//            actions[BhaashikMainAction.LOGOUT].setEnabled(true);
////            actions[BhaashikMainAction.RUN_CLIENT].setEnabled(true);
//
//            loggedIn = true;
...
//
////	String location = "rmi://" + server + "/BhaashikServer";
////	try {
////		bhaashikServerRI = (BhaashikServerRI) Naming.lookup(location);
...
////		ex.printStackTrace();
////	}
////        bhaashikServerRI = (BhaashikServerRI) ServerType.getNewServer(ServerType.SANCHAY_SERVER, server, true);
...
//
////	if(bhaashikServerRI == null || userManagerRI == null)
...
////        try
////        {
//////            result = (String) bhaashikServerRI.checkConnection();
////            result = (String) userManagerRI.checkConnection();
////            statusJLabel.setText(result + GlobalProperties.getIntlString("_to_") + server);
////
////            connected = true;
////            actions[BhaashikMainAction.CONNECT_REMOTE].setEnabled(false);
////            actions[BhaashikMainAction.DISCONNECT_REMOTE].setEnabled(true);
//////            actions[BhaashikMainAction.LOGIN].setEnabled(true);
////            actions[BhaashikMainAction.LOGOUT].setEnabled(false);
////
////        } catch (Exception ex) {
////	    disconnectRemote(null);
////            statusJLabel.setText(result);
...
//        bhaashikServerRI = (BhaashikServerRI) ServerType.getNewServer(ServerType.SANCHAY_SERVER, "", false);
        userManagerRI = (UserManagerRI) ServerType.getNewServer(ServerType.USER_MANAGER, "", false);

        connected = false;
        loggedIn = false;

        actions[BhaashikMainAction.CONNECT_REMOTE].setEnabled(true);
        actions[BhaashikMainAction.DISCONNECT_REMOTE].setEnabled(false);
//        actions[BhaashikMainAction.LOGIN].setEnabled(true);
        actions[BhaashikMainAction.LOGOUT].setEnabled(false);
...
//    public void bhaashikLogin(ActionEvent e)
//    {
//        JDialog loginDialog = new JDialog(this, GlobalProperties.getIntlString("Login_to_Bhaashik"), true);
//
...
//    }
    ...
    public void bhaashikLogout(ActionEvent e)
    {
//        closeAllClients(null);
...
//        actions[BhaashikMainAction.LOGIN].setEnabled(true);
    ...
    public void windowClosing(WindowEvent e) {
        BhaashikJDialog dlg = (BhaashikJDialog) e.getSource();
    ...
    public void fireEvent(BhaashikEvent evt)
    {
        ...
        {
            if (listeners[i] instanceof BhaashikMainEventListener
                    && evt instanceof BhaashikMainEvent)
            {
                if(evt.getEventID() == BhaashikMainEvent.OPEN_TAB)
                    ((BhaashikMainEventListener) listeners[i]).openTab((BhaashikMainEvent) evt);
                else if(evt.getEventID() == BhaashikMainEvent.DISPLAY_FILE)
                    ((BhaashikMainEventListener) listeners[i]).displayFile((BhaashikMainEvent) evt);
            }
        }
    }
    ...
    public static BhaashikRemoteWorkJPanel getSSHClientJPanel()
    {
        return remoteClientJPanel;
    }
