 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class PropertiesManager extends AggregateResourceImpl implements Serializable, BhaashikProperties, BhaashikDOMElement
{
    ...
    public BhaashikProperties getPropertyContainer(String p /* property key */, PropertyType pt)
    {
        Hashtable ht = getPCHashtable(pt);
        
        if(ht != null)
            return (BhaashikProperties) ht.get(p);
        
        return null;
    }
    
    public int addPropertyContainer(String p /* property key */, BhaashikProperties v /*property value */, PropertyType pt)
    {
        Hashtable ht = getPCHashtable(pt);
    ...
    public BhaashikProperties removePropertyContainer(String p /* property key */, PropertyType pt)
    {
        Hashtable ht = getPCHashtable(pt);
        
        if(ht != null)
        {
            return (BhaashikProperties) ht.remove(p);
        }
    ...
    public void savePropertyContainer(String p /* property key */, PropertyType pt) throws Exception
    {
        ...
        BhaashikProperties spc = getPropertyContainer(p, pt);
        spc.save(path, cs);
    }
    ...
    public static LinkedHashMap readMany(String f, String charset) throws FileNotFoundException, IOException
    {
        ...
        while((line = lnReader.readLine()) != null)
        {
            ...
            {
                if(line.startsWith(GlobalProperties.getIntlString("PropertiesManagerBegin::")))
                {
                    splitstr = line.split("::");
                    pmName = splitstr[1];
                ...
                else if(line.startsWith(GlobalProperties.getIntlString("PropertiesManagerEnd")))
                {
    ...
    public void print(PrintStream ps)
    {
        ...
        while(enm.hasMoreElements())
        {
            ...
            while(enm1.hasMoreElements())
            {
                ...
                BhaashikProperties sp = getPropertyContainer(key, pt);
                ps.println("#" + pt + "::" + key);
                sp.print(ps);
                ps.println("#############################################");
            }
    ...
    public static void printMany(ConcurrentLinkedHashMap propmans, PrintStream ps)
    {
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            ps.println(GlobalProperties.getIntlString("PropertiesManagerBegin::") + tblName);
	    pm.print(ps);
            ps.println(GlobalProperties.getIntlString("PropertiesManagerEnd"));
        }
    }

    public static void printManyXML(Hashtable propmans, PrintStream ps)
    {
        DOMElement domElementMany = new DOMElement(GlobalProperties.getIntlString("MultiPropertiesManager"));
	...
	String sfname = GlobalProperties.getHomeDirectory() + "/" + "PropertiesManager-tmp.tmp";
...
//        domElementConst.add(((BhaashikDOMElement) taskProps).getDOMElement());
//        
...
//                String key = (String) enm1.nextElement();
//                
////                DOMElement domElementProps = new DOMElement("BhaashikProperties");
...
//                BhaashikProperties sp = getPropertyContainer(key, pt);
//                
//                domElementProps.add(((BhaashikDOMElement) sp).getDOMElement());
...
//    }
    public DOMElement getDOMElement() {
        DOMElement domElement = new DOMElement(GlobalProperties.getIntlString("PropertiesManager"));
        
        DOMElement domElementConst = new DOMElement(GlobalProperties.getIntlString("Constants"));
        ...
        while(enm.hasMoreElements())
        {
            ...
            while(enm1.hasMoreElements())
            {
             ...
             //   DOMAttribute attribKey = new DOMAttribute(domElementProps, new org.dom4j.QName("key"), key);
                ...
                domElementProps.addAttribute(GlobalProperties.getIntlString("key"), key);
                BhaashikProperties sp = getPropertyContainer(key, pt);
    ...
    {
        try {
            PropertiesManager pm = new PropertiesManager(GlobalProperties.getHomeDirectory() + "/" + "workspace/server-props.txt", GlobalProperties.getIntlString("UTF-8"));
//            pm.print(System.out);
//            PropertiesTable taskProps = (PropertiesTable) pm.getProperties();
            PropertiesTable users = (PropertiesTable) pm.getPropertyContainer(GlobalProperties.getIntlString("users"), PropertyType.PROPERTY_TABLE);
...
//            String outColNames[] = {"FilePath", "Charset"};
            ...
            String colNames[] = {GlobalProperties.getIntlString("User"), "UserType"};
            String vals[] = {GlobalProperties.getIntlString("anil"), GlobalProperties.getIntlString("validator")};
            String outColNames[] = {"UserType", GlobalProperties.getIntlString("Password")};
            String outVals[] = {GlobalProperties.getIntlString("admin"), "a"};
