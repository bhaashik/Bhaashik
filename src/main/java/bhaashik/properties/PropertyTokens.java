 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class PropertyTokens extends ResourceImpl implements Serializable, BhaashikProperties, BhaashikDOMElement, Cloneable
{
    ...
    public void makeUnique()
    {
        ...
        {
            unique.put(getToken(i), Boolean.valueOf(true));
        }
    ...
    public Vector getTypes()
    {
        ...
        for(int i = 0; i < count; i++)
            types.put(getToken(i), Integer.valueOf(1));
...
//            if((!line.startsWith("#")) && line.equals("") == false)
            if((!line.startsWith("#")) && line != "")
            {
                ...
                else if(line.startsWith(GlobalProperties.getIntlString("Column_Count::")) == false)
                    addToken(line);
            }
        }
    ...
    public static ConcurrentLinkedHashMap readMany(String f, String charset) throws FileNotFoundException, IOException
    {
        ...
        while((line = lnReader.readLine()) != null)
        {
            if((!line.startsWith("#")) && line.equals("") == false)
            {
                if(line.startsWith(GlobalProperties.getIntlString("PropertyTokensBegin::")))
                {
                    splitstr = line.split("::");
                    ptName = splitstr[1];
                ...
                else if(line.startsWith(GlobalProperties.getIntlString("PropertyTokensEnd")))
                {
	...
	if(getPropertyName().equals("") == false)
	    ps.println(GlobalProperties.getIntlString("Column_Names::") + getPropertyName());
    ...
    public static void printMany(ConcurrentLinkedHashMap ptokens, PrintStream ps)
    {
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            ps.println(GlobalProperties.getIntlString("PropertyTokensBegin::") + ptName);
	    ptoken.print(ps);
            ps.println(GlobalProperties.getIntlString("PropertyTokensEnd"));
        }
    }
    ...
    public static void printManyXML(ConcurrentLinkedHashMap ptokens, PrintStream ps)
    {
        ...
        DOMElement domElementMany = new DOMElement(GlobalProperties.getIntlString("PropertyTokensMany"));
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            DOMAttribute attribPT = new DOMAttribute(domElement, new org.dom4j.QName(GlobalProperties.getIntlString("name")), ptName);
    ...
    public static BhaashikTableModel convertToBhaashikTableModel(PropertyTokens pt)
    {
        int tcount = pt.countTokens();
        
        BhaashikTableModel table = new BhaashikTableModel(tcount, 1);
    ...
    public static PropertyTokens convertToPropertyTokens(BhaashikTableModel stm)
    {
        PropertyTokens pt = new PropertyTokens();
...
//    }
    public DOMElement getDOMElement() {
        DOMElement domElement = new DOMElement(GlobalProperties.getIntlString("PropertyTokens"));
        ...
        {
            String token = getToken(i);
            DOMElement tokenElement = new DOMElement(GlobalProperties.getIntlString("PropertyToken"));
           // DOMAttribute attribToken = new DOMAttribute(tokenElement, new org.dom4j.QName("token"), token);
            tokenElement.addAttribute(GlobalProperties.getIntlString("token"), token);
            domElement.add(tokenElement);
        }
...
//    }
    public void readXML(Element domElement) {
        ...
        NodeList elements = domElement.getElementsByTagName(GlobalProperties.getIntlString("PropertyToken"));
        int count = elements.getLength();
     //   System.out.println("I am adding tokens here: ");
        for (int i = 0; i < count; i++)
        {
            org.w3c.dom.Element kvElement = (org.w3c.dom.Element) elements.item(i);
            String token = kvElement.getAttribute(GlobalProperties.getIntlString("token"));
...
//
        ...
        try
        {
            pt.read("/home/anil/bhaash/Bhaashik/data/transliteration/translit-test-data-hindi.txt", "UTF-8");

            ptOut = pt.getSpacedOutStrings(pt);

            ptOut.save("/home/anil/bhaash/Bhaashik/data/transliteration/translit-test-data-hindi-spaced-out.txt", "UTF-8");
//            kvp.read("/home/anil/joro/data/two-telugu-books-1016-1018-word-list-3.txt", bhaashik.GlobalProperties.getIntlString("UTF-8"));
...
//            }
            ...
            kvp.save("/home/anil/joro/data/two-telugu-books-1016-1018-word-list-4.txt", GlobalProperties.getIntlString("UTF-8"));
